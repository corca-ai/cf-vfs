...
`read` and `read -r` must use the managed shell-input cursor rather than taking
a raw reader and discarding the suffix of a chunk after newline. Wrap every
root, pipeline, here-document, here-string, and input-redirection stream at its
shell fd boundary. Tests must cover several records in one chunk, a UTF-8 code point
