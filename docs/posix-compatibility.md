...
## Language utilities with dedicated oracles
...
The opt-in AWK profile is compared against the repository's pinned BusyBox
1.37.0 image in `test/fixtures/awk-compat.json`. Its cases cover the supported
common subset: record and field variables, `BEGIN`/`END`, ERE and expression
patterns, accumulation and updates, control flow, field rewriting, formatting,
string functions, associative arrays, all three loop forms, range patterns,
substitution and splitting, `-F`/`-v`, numeric-string coercion, and exit status.
VFS-backed `-f` has direct unit coverage because the container fixture has no
access to the caller's virtual filesystem. The profile deliberately excludes
user functions, subprocesses, `getline`, and locale-dependent behavior; see
[the AWK profile](commands.md#the-awk-profile).
